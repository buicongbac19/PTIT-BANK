...
def get_latest_transactions(account_id, limit=5):
    ...
    account_number = Account.query.with_entities(Account.accountNumber).filter_by(AccountID=account_id).first()[0]
    ...
    transactions = (
        ...
        .join(RecipientAccount, Transaction.recipientAccountNumber == RecipientAccount.accountNumber)
    ...
    result = []
    for txn, sender_username, first_name, last_name, recipient_account_number in transactions:
        recipient_name = f"{first_name} {last_name}" if first_name and last_name else "Không xác định"
        result.append((txn, sender_username, recipient_name, recipient_account_number))  # Trả về tuple phù hợp với template
...
def get_transaction():
    ...
    if request.method == "POST":
        ...
        transactions = (
            db.session.query(
                ...
                case(
                    (Transaction.senderAccountNumber == account.accountNumber, 'outgoing'),
                    else_='incoming'
                ).label('transaction_type')
            )
            .join(SenderAccount, Transaction.senderAccountNumber == SenderAccount.accountNumber)
            .join(RecipientAccount, Transaction.recipientAccountNumber == RecipientAccount.accountNumber)
            .join(SenderCustomer, SenderAccount.CustomerID == SenderCustomer.CustomerID)
            .join(RecipientCustomer, RecipientAccount.CustomerID == RecipientCustomer.CustomerID)
            .filter(
                or_(
    ...
    return render_template("profile.html", transactions=transactions, account=account, customer=customer)
